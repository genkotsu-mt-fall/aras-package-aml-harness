#!/usr/bin/env bash
# scripts/11-review-test-cases.sh
...
OUTPUT_FILE="$REPO_ROOT/$TASK_DIR/test-cases-review${REVIEW_NO}.md"
...
echo "Review output: $OUTPUT_FILE" >&2

"$SCRIPT_DIR/codex-run.sh" \
  read-only \
  prompts/11-review-test-cases.md \
  "$TASK_DIR" \
  "$REVIEW_NO" \
  "$EXTRA_INSTRUCTION" \
  > "$OUTPUT_FILE"
